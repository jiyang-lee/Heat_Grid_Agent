...
- `risk_score` 또는 `risk_probability`는 고장신고 전 위험 패턴과의 유사성 또는 위험 가능성을 나타낸다.
- 이 값은 LightGBM이 산출하는 지도학습 기반 위험도이며, 고장 확정값이 아니다.
...
- `risk_score` 또는 `risk_probability` = 고장신고 전 위험 패턴과 얼마나 유사한가
