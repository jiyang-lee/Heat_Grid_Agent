...
COLUMN_GROUPS = {
    ...
    "priority_score": ("Priority", "최종 agent용 restored Risk/pre-event gate v4 priority score"),
    "priority_level": ("Priority", "최종 agent용 priority level"),
    "priority_source": ("Priority", "최종 priority 생성 방식"),
    "policy_version": ("Priority", "공식 priority 운영 정책 버전"),
    "current_best_weight": ("Priority", "가중 결합 정책일 때의 current-best 가중치; 공식 v4는 비어 있음"),
    "m1_specialist_weight": ("Priority", "가중 결합 정책일 때의 M1 specialist 가중치; 공식 v4는 비어 있음"),
    "decision_basis": ("Priority", "공식 Risk/pre-event gate 조건과 level 임계값"),
    ...
    "m1_hybrid_priority_score": ("Priority", "비교용 요청 v2 weighted hybrid score"),
    "m1_hybrid_priority_level": ("Priority", "비교용 요청 v2 hybrid level"),
    "m1_evidence_pre_event_score": ("Priority", "pre-event probability를 0~100으로 변환한 evidence"),
    "m1_evidence_leadtime_score": ("Priority", "leadtime urgency에 운영 offset을 반영한 evidence"),
    "m1_risk_pre_event_priority_score": ("Priority", "복원 Risk와 pre-event evidence 중 큰 공식 v4 score"),
    "m1_risk_pre_event_priority_level": ("Priority", "공식 Risk/pre-event gate v4 priority level"),
    "m1_risk_pre_event_trigger": ("Priority", "high gate를 통과시킨 Risk/pre-event 근거"),
    "m1_evidence_priority_score": ("Priority", "이전 v3 pre-event/leadtime evidence score"),
    "m1_evidence_priority_level": ("Priority", "이전 v3 evidence gate priority level"),
    "m1_evidence_trigger": ("Priority", "이전 v3 high gate의 pre-event/leadtime 근거"),
...
FINAL_AGENT_CARD_GROUPS = {
    ...
    "최종 M1 Risk/pre-event gate priority contract": [
        "priority_score",
        "priority_level",
        "priority_source",
        "policy_version",
...
FINAL_AGENT_GROUP_ORIGIN = {
    ...
    "최종 M1 Risk/pre-event gate priority contract": "최종 agent 계약 v4: restored Risk 0.78 OR pre-event 0.99, label-free gate",
...
FINAL_AGENT_GROUP_USAGE = {
    ...
    "최종 M1 Risk/pre-event gate priority contract": "agent UI/API가 정렬과 level 판단에 우선 사용하는 v4 필드; v3와 v2는 비교용으로 보존",
        ...
        rows.append(
            {
                ...
                if group == "최종 M1 Risk/pre-event gate priority contract"
