...
def materialize_current_best_model_artifacts() -> dict[str, object]:
    """Refresh current-best model artifacts from a discovered source project when available.

    Risk/leadtime model training remains owned by the current-best project. This
    package keeps the promoted artifacts locally so the M1 handoff can be
    validated and reproduced without hard-coded absolute paths.
    """
    config.ensure_dirs()
    specs = [
        (
            "risk_model_best",
            [config.SOURCE_RISK_MODEL_PATH],
            config.RISK_MODEL_PATH,
        ),
        (
            "risk_model_best_metadata",
            [config.SOURCE_RISK_METADATA_PATH, config.PACKAGED_RISK_METADATA_PATH],
            config.RISK_METADATA_PATH,
        ),
        (
            "leadtime_model_best",
            [config.SOURCE_LEADTIME_MODEL_PATH],
            config.LEADTIME_MODEL_PATH,
        ),
        (
            "leadtime_model_best_metadata",
            [config.SOURCE_LEADTIME_METADATA_PATH, config.PACKAGED_LEADTIME_METADATA_PATH],
            config.LEADTIME_METADATA_PATH,
        ),
        (
            "priority_engine_best_metadata",
            [config.SOURCE_PRIORITY_METADATA_PATH, config.PACKAGED_PRIORITY_METADATA_PATH],
    ...
    payload = {
        ...
        "artifacts": artifacts,
        "note": "Anomaly and M1 specialist artifacts are regenerated by their own pipeline steps. Current-best risk/leadtime artifacts are refreshed from the source project when available, otherwise packaged artifacts are kept.",
    }
