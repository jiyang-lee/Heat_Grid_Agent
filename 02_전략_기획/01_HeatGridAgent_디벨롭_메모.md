# 주제 17 — HeatGrid : 도시 열인프라 다중 엔티티 정비 우선순위 PdM Agent

> **한 줄 정의**
> 공개 지역난방 기계실 데이터(PreDist)를 이용해 여러 기계실의 운영 상태를 실시간 스트림처럼 재현하고, 각 기계실의 **이상 징후·고장 리드타임·영향도**를 추정한 뒤, **제한된 출동 자원 아래 어떤 기계실을 먼저 점검해야 하는지 자동 우선순위화**하는 도시 열인프라 PdM 코파일럿.

---

## 1. 재정의된 포지셔닝

기존 설명의 약점은 `지역난방 기계실 예지보전`이 BMS 계열 주제와 비슷하게 들리고, 도메인 설명 난도가 높다는 점이다.  
이 문서는 HeatGrid를 **건물 설비 모니터링**이 아니라, **도시 단위로 분산된 다중 엔티티 인프라의 출동 우선순위 결정 문제**로 다시 정의한다.

- **핵심 문제:** 고장을 예측하는 것보다, **수십~수백 개 기계실 중 오늘 어디를 먼저 보내야 하는지 결정하는 것**
- **차별 축:** 단일 설비 제어가 아니라 **다중 엔티티 우선순위화**
...
기계실 스트림 → 이상 징후/리드타임 추정 → 영향도 반영 위험도 계산 → 출동 후보 우선순위화 → 작업지시 생성 → 조치 후 리스크 변화 확인
