...
def _condition_row(row: dict[str, Any], condition: str) -> dict[str, Any]:
...
def dry_run(
    ...
    for selection_name, row, reason in rows:
        effective_row = _condition_row(row, condition)
        payload = build_generation_input(
            effective_row,
            include_retrieval=condition == "with_rag",
        )
        ...
        records.append({
            ...
            "retrieval_hit_at_5": effective_row.get("retrieval_hit_at_5"),
            "input_context_count": len(effective_row.get("retrieved_contexts") or []),
...
def generate_rows(
    ...
    default_output = (
        Path("rag_evaluation/results/answer_generation_no_rag.jsonl")
        if condition == "no_rag"
        else config.output_path
    )
    target_output_path = Path(output_path) if output_path else default_output
    if not append:
    ...
    for _, row, _ in rows:
        ...
        cited_chunk_ids, policy_warnings = enforce_citation_policy(effective_row, cited_chunk_ids)
        output["cited_chunk_ids"] = cited_chunk_ids
        output_warnings = policy_warnings + validate_generation_output(effective_row, output)
        ...
        write_jsonl(target_output_path, [record], append=True)
...
def main() -> int:
    ...
    if args.resume:
        if not args.output_path:
            raise SystemExit("--resume requires --output-path")
        existing_path = Path(args.output_path)
        existing_ids = (
            {row.get("case_id") for row in load_jsonl(existing_path)}
            if existing_path.exists()
            else set()
        )
        chosen = [item for item in chosen if item[1].get("case_id") not in existing_ids]
        args.append = True

    if args.dry_run:
        summary = dry_run(chosen, prompt, condition=args.condition)
    else:
        if args.all:
