...
def build_generation_input(
    ...
    *,
    ...
    contexts = []
    for ctx in (row.get("retrieved_contexts") or []) if include_retrieval else []:
        contexts.append({
            "rank": ctx.get("rank"),
    ...
    return {
        ...
        "metadata": {
            ...
            "retrieval_hit_at_5": row.get("retrieval_hit_at_5") if include_retrieval else False,
            "evaluation_condition": "with_rag" if include_retrieval else "no_rag",
        },
...
def make_result_record(
    ...
    evaluation_condition: str = "with_rag",
) -> dict[str, Any]:
    usage = usage or {}
