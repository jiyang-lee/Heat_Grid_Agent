...
> `.env`는 커밋되지 않습니다. 파일이 없어도 기본 CARTO 지도와 `http://127.0.0.1:8003` 실백엔드를 사용합니다. 사용자 지정 지도나 백엔드 주소가 필요할 때만 `.env.example`을 복사해 설정합니다.
...
> ⚠️ 프로덕션 빌드에는 dev 서버의 `/api` 프록시가 **없다**. 배포본은 백엔드 공개 호스팅과 `/api` rewrite 또는 절대 URL + CORS 구성이 필요하다.
...
- API 계약/타입: `src/api/contracts.ts` (백엔드 `schemas.py`와 1:1). 실 API 진입점은 `src/api/backend.ts`.
