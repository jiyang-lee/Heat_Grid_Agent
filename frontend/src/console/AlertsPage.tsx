...
import { Button, ApiState, MetricCard, Sparkline, StatusBadge, SurfaceCard, type Tone } from './ui'
...
export function AlertsPage({ onRunCreated }: Props) {
  ...
  const rows = useMemo(() => (alerts.data ?? []).filter((alert) => `${alert.manufacturer_id} ${alert.substation_id} ${alert.enqueue_reason}`.toLowerCase().includes(search.toLowerCase())), [alerts.data, search])
