  ...
  return <div className="page-stack settings-page">
    ...
    <div className="settings-layout">
      <div className="settings-main">
        ...
        {tab === '업무 설정' && <SurfaceCard title="AI 조치와 문서 업무 설정"><div className="settings-form"><section><h3>AI 조치 기본 화면</h3><div className="form-grid"><label>기본 탭<select defaultValue="runs"><option value="runs">조치 계획</option><option value="reports">AI 보고서</option><option value="orders">작업지시서</option></select></label><label>실행 현황 정렬<select defaultValue="latest"><option value="latest">최근 실행 순</option></select></label><label>자동 새로고침<select defaultValue="minute"><option value="minute">1분</option></select></label></div></section><section><h3>문서 발행 원칙</h3><div className="form-grid"><label className="check-label"><input defaultChecked type="checkbox" />사람 승인 후 작업지시서 발행</label><label className="check-label"><input defaultChecked type="checkbox" />보고서 파일명에 날짜 포함</label><label>PDF 출력 형식<select defaultValue="a4"><option value="a4">A4 세로</option></select></label></div><p>작업지시서 승인 상태와 보고서 파일은 서버 실행 기록에 저장됩니다.</p></section></div></SurfaceCard>}
      </div>
