...
export function ScenarioAiPage() {
  ...
  return <div className="page-stack activity-page scenario-ai-page">
    ...
    {tab === '작업지시서' && <ScenarioWorkOrderWorkspace alert={alert} key={state.activeDocumentGroupId ?? 'empty-order'} onAccept={acceptWorkOrder} onAppendMessages={scenario.appendWorkOrderMessages} onAppendRevision={scenario.appendWorkOrderRevision} onCreateReport={() => { createReportDraft(); setTab('보고서') }} onOpenAnalysis={() => setTab('계획서')} onSelectDocumentGroup={scenario.selectDocumentGroup} onSelectVersion={scenario.selectWorkOrderVersion} onUpdateContent={scenario.updateWorkOrderContent} state={state} />}
