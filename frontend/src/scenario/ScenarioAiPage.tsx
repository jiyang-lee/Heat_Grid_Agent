...
export function ScenarioAiPage() {
  ...
  return <div className="page-stack activity-page scenario-ai-page">
    ...
    {tab === '작업지시서' && <ScenarioWorkOrderWorkspace alert={alert} key={state.activeDocumentGroupId ?? 'empty-order'} onAccept={acceptWorkOrder} onAppendManualRevision={scenario.appendManualWorkOrderRevision} onAppendMessages={scenario.appendWorkOrderMessages} onAppendRevision={scenario.appendWorkOrderRevision} onCreateReport={() => { createReportDraft(); setTab('보고서') }} onOpenAnalysis={() => setTab('계획서')} onSelectDocumentGroup={scenario.selectDocumentGroup} onSelectVersion={scenario.selectWorkOrderVersion} onUpdateContent={scenario.updateWorkOrderContent} state={state} />}
    {tab === '보고서' && <ScenarioReportWorkspace activeGroupId={state.activeDocumentGroupId} alert={alert} groups={state.documentGroups} key={state.activeDocumentGroupId ?? 'empty-report'} messages={state.reportMessages} onComplete={scenario.completeReport} onCreateDraft={createReportDraft} onOpenWorkOrders={() => setTab('작업지시서')} onPostMessage={scenario.postReportMessage} onSave={scenario.saveReportDraft} onSelectDocumentGroup={scenario.selectDocumentGroup} order={acceptedOrder} report={state.report} />}
  </div>
}
