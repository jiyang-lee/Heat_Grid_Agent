...
export function ScenarioAiPage() {
  ...
  return <div className="page-stack activity-page scenario-ai-page">
    ...
    {tab === '보고서' && <ScenarioReportWorkspace activeGroupId={state.activeDocumentGroupId} alert={alert} groups={state.documentGroups} key={state.activeDocumentGroupId ?? 'empty-report'} onComplete={scenario.completeReport} onCreateDraft={createReportDraft} onOpenWorkOrders={() => setTab('작업지시서')} onSave={scenario.saveReportDraft} onSelectDocumentGroup={scenario.selectDocumentGroup} order={acceptedOrder} report={state.report} />}
  </div>
}
