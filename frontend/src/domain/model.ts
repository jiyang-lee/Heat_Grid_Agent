...
export const complexNameOf = (substationId: number | null, _manufacturerId?: string | null): string =>
  substationId == null ? '알 수 없는 단지' : complexById.get(substationId)?.name ?? `단지 ${substationId}`
