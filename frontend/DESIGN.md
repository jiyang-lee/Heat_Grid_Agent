...
- Queue motion: a new `final-test` entry auto-expands the screenshot-4 progress toast. `requestedAt <= now < readyAt` is `분석 중`; the shared three-second completion time then becomes `완료` without blocking the main thread. Older persisted five-second timestamps are normalized on load.
- Document flow: the screenshot-5 analysis detail opens the screenshot-6 work-order shell after completion. Version v1 remains the DB-backed source, while direct edits create session-only v2/v3 content and enable browser XLSX/DOCX downloads. Local session acceptance unlocks report generation, and the screenshot-7 report shell uses local session approval only.
...
- Conversation anatomy: one `demo_id` owns its session transcript in `sessionStorage`; the `HeatGrid 운영 도우미` header, scrollable log, suggested prompts, guarded scripted replies and composer survive work-order/report tab changes. Chat copy is normalized to `기계실` and does not expose implementation or project-only labels.
