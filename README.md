...
-> SQL evidence 조회
-> RAG/단지/기상 참고자료 조회
-> 활성 모델 재검증과 근거 충족도 반복 판단
-> 필요 시 내부검색 확대·외부 근거 후보·모델 재실행
...
| `simulator/versions/v2_postgres_react_ops/backend/` | FastAPI entrypoint, route, frontend bridge, legacy 실행 명령 호환 wrapper. |
...
| `src/heatgrid_ops/` | LangGraph agent runtime, backend service adapter, 보고서 graph의 canonical Python 패키지. |
...
| 에이전트 반복 이력 | `GET /api/agent-runs/{run_id}/iterations` |
| 사람 검수와 근거 후보 | `GET /api/review-tasks`, `GET /api/evidence-candidates` |
| 자동화 정책 | `GET/PATCH /api/automation-policy` |
| 재학습과 모델 승격 | `GET/POST /api/retrain-jobs`, `GET /api/model-candidates` |
| 서버 상태 | `GET /health` |
...
프론트는 기존 결과 계약을 유지하면서 반복 이력, 검수함, 근거 승인, 재학습, 모델 승격, 자동화 정책 API를 직접 사용한다.

| 화면 | 내용 |
|---|---|
| 지도 관제 | 세종 1생활권 31개 단지 지도와 기계실 상세 화면. |
| 운영 콘솔 | `/api/alerts` 알림 큐, ack/resolve, agent run 실행, SSE timeline, token/cost, v4 작업 지시 결과 표시. |
| 검수함 | 최종 출력과 모델 불일치의 승인·반려·교정, 학습 피드백 생성. |
| 근거 승인 | 외부 근거 원문 검수와 승인 자료의 RAG 적재. |
| 재학습·모델 | 재학습 승인, 실행 상태, 후보 모델 최종 승격. |
| 자동화 정책 | 전면 검수·검수 보조·제한 자동 단계와 자격 기준 관리. |
...
| `docs/14_AGENT_RECURSIVE_AUTOMATION.md` | 재귀 루프, 모델 재검증, 사람 검수, 재학습과 승격 전체 구조. |
| `docs/15_PRIORITY_EVALUATION_SNAPSHOT.md` | 동일 기준 시점의 31개 Substation 평가, 순위, 지도·알림·에이전트 연결 구조. |

## 현재 작업 기준

`develop2-loop`에서 백엔드와 프론트의 재귀 자동화 흐름을 함께 관리한다.

| 담당 | 주 작업 경로 |
|---|---|
| 프론트 | `frontend/**` |
| 백엔드 | `src/heatgrid_ops/**`, `simulator/versions/v2_postgres_react_ops/backend/**`, `src/heatgrid_rag/**`, `src/heatgrid_weather/**` |
