...
def test_mixed_model_usage_uses_each_models_pricing() -> None:
    config = AgentRuntimeConfig(
        ...
        model_pricing_overrides=(
            ModelPricing(
                ...
                pricing_source="quality-pricing",
            ),
        ),
    )
    usage = usage_with_totals(
        TokenUsage(
            calls=[
                TokenCall(
                    model="gpt-5.4-mini",
    ...
    assert usage.cost_estimate.model == "gpt-5.4-mini+gpt-5.4"
    assert usage.cost_estimate.total_cost_usd == pytest.approx(0.000794)
    assert usage.cost_estimate.pricing_source == (
        "mini-pricing | quality-pricing"
    )
