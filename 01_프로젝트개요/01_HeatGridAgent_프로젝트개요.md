# 주제 02 — HeatGrid : 지역난방 인프라(기계실·열수송) 예지보전 AI Agent

> **한 줄 정의**
> 공개 지역난방 기계실(substation) 운영 데이터를 실시간 IoT 스트림으로 재현하고, ML/DL로 **고장을 고객 신고 전에 예측**한 뒤, LangGraph AI Agent가 **이상 진단 → 정비 우선순위 → 출동 지시서**까지 자동 생성하는 "도시 열에너지 인프라 예지보전 코파일럿".
...
|---|---|
| 프로젝트명 | **HeatGrid** |
| 프로젝트 주제 | AIoT 기반 지역난방 기계실 예지보전 & 정비 자동화 |
...
### 2.1 개발 배경
- **AX·인프라 전환:** 도시 인프라 운영이 "신고 받고 출동"에서 **"데이터로 미리 감지해 선제 정비"**로 이동. 에너지 시스템 이상탐지 오픈소스(EnergyFaultDetector)와 공개 라벨 데이터가 등장하며 진입장벽이 낮아짐.
- **분산·대수의 운영:** 기계실이 수백 개로 분산되어, 사람이 전수 모니터링은 불가 → 우선순위화가 핵심.
- **고객 신고 의존:** 현재는 고객이 "추워요"라고 신고해야 인지 → 이미 서비스 품질이 깨진 시점.

### 2.2 문제 정의
> **"도시 열 공급은 수백 개 기계실에 의존하지만, 이상은 보통 '고객이 추위를 느낀 뒤'에야 발견된다."**

- **누가 쓰는가(Who):** 지역난방 운영센터 관제사, 정비팀, 설비 엔지니어.
- **왜 필요한가(Why):** ① 고장의 상당수는 운영데이터에 전조가 있으나 임계 알람으로는 못 잡는다. ② 출동 자원은 한정 → 위험·영향 순 우선순위가 필요. ③ 동절기 공급 중단은 직접적 민원·안전 문제.
- **무엇을 해결하는가(What):** 기계실 스트림 → **고장 사전 예측** → **이상 진단·우선순위화** → **출동 지시서** 폐루프 자동화.

### 2.3 핵심 기능 요약
| # | 기능 | 한 줄 설명 |
|---|---|---|
| 1 | 다수 기계실 인프라 관제 | 네트워크 전체 상태를 지도/그리드로 시각화 |
| 2 | 고장 사전 예측 | 정상행동 모델 대비 이탈로 조기 탐지 |
| 3 | 이상 진단·리드타임 추정 | 어떤 고장·언제쯤 발생 가능성 |
| 4 | AI Agent 정비 자동화 | LangGraph: 감지→진단→우선순위→출동 지시서 |
