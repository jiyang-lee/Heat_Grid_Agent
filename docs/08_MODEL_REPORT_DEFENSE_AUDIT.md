...
| 과거 bridge 26개 누락 해소 | 포함 | `compare/m1_threshold_weight_rationale_report.ipynb` 섹션 3, `output/reports/row_flow_summary.csv`, `output/reports/key_coverage_by_artifact.csv`, `output/reports/missing_agent_windows.csv` | 보존된 상위 artifact의 M1 score는 1226개지만 현재 패키지는 canonical 1252개를 모두 보존 | final agent 1252개, 누락 0개 |
| M1-only scope | 포함 | `output/reports/m1_scope_audit.md`, `docs/model/MODEL_INVENTORY_KO.md`, `compare` notebooks | M2 calibration 수치가 없음 | 현재 검증은 `manufacturer 1` 기준. M2 또는 전체 제조사 일반 성능으로 표현 금지. M2 적용 시 별도 calibration/validation 필요 |
...
| risk level applied threshold | 포함 | `compare/m1_threshold_weight_rationale_report.ipynb` 섹션 3/5, `output/reports/risk_level_actual_summary.csv`, `output/reports/risk_threshold_actual_values.csv` | 과거 fallback 설명의 0.44를 현재 active M1 threshold처럼 말하면 안 됨 | 복원 M1 output 기준은 medium 0.22, high 0.78, critical 0.92. high 0.78은 validation FPR 5% cap 아래에서 선택 |
...
| M1 Specialist 가중치 | 부분 포함 | `compare/m1_threshold_weight_rationale_report.ipynb` 섹션 8, `output/reports/m1_specialist_priority_weight_ablation.csv`, `output/reports/m1_specialist_priority_weight_grid.csv` | 0.55/0.30/0.15가 metric-only best는 아님 | live 경로와 맞추기 위해 group weight는 unknown_review=0.1로 고정 |
| fault_group_weight | 반영 | `models/m1_specialist/m1_specialist_gate_metadata.json`, `output/m1_specialist_scores.csv` | `fault_label`은 live inference에 없음 | 공식 점수에서 label-derived group을 제거하고 0.1 상수로 통일 |
| Risk/pre-event gate v4 | 포함 | `output/reports/m1_risk_pre_event_gate_threshold_sweep.csv`, `m1_specialist_vs_current_best_comparison.csv` | validation 이벤트가 3건으로 작음 | restored Risk 0.78 OR pre-event 0.99를 공식화하고 v3/v2/v1은 비교·rollback으로 보존 |
...
| agent card 컬럼 계약 | 포함 | `docs/02_AGENT_OUTPUT_CONTRACT.md`, `docs/handoff/AGENT_HANDOFF_KO.md`, `output/agent/agent_card_column_groups_ko.md` | 최종 v4 card와 M1 parallel card를 혼동하면 안 됨 | 최종 card는 1252 rows / 67 columns. M1 parallel card는 1252 rows / 29 columns이며 M1-only evidence |
...
- 보존된 상위 artifact M1 score 1226개와 현재 final M1 agent 1252개의 차이를 구분하고, 현재 누락은 0개임을 보고서에 반영하라.
...
- 공식 Risk/pre-event gate v4와 이전 v3, 요청 v2 `0.72/0.28`, rollback v1 `0.65/0.35`를 구분하라.
...
- `risk_scores.csv`의 applied threshold 컬럼을 확인하고, 실제 M1 output 기준 0.22/0.78/0.92를 문서에 반영하라.
- 최종 agent card 67 columns와 M1 specialist parallel card 29 columns를 구분해서 설명하라.
...
| 항목 | 보완 위치 | 추가할 내용 |
|---|---|---|
| risk/leadtime 재현 가능성 | `docs/model/MODEL_INVENTORY_KO.md`, `docs/07_HANDOFF_FILE_INDEX.md` | 모델 파일은 포함됐지만 기본 실행은 score bridge라는 점 |
...
| 과거 bridge 누락 해소 | `compare` notebooks, `docs/07_HANDOFF_FILE_INDEX.md` | 상위 artifact M1 1226, 현재 final 1252, 누락 0 |
| M1-only scope | 모든 발표/보고서 요약 | M2 일반화 금지, M2 calibration 필요 |
| priority 정책 표현 | `compare/m1_threshold_weight_rationale_report.ipynb`, 발표본 | 공식 v4는 restored Risk 0.78 OR pre-event 0.99, 이전 v3·요청 v2·v1은 비교 기준 |
| risk applied threshold | `docs/model/MODEL_INVENTORY_KO.md`, `docs/handoff/AGENT_HANDOFF_KO.md`, `compare/m1_threshold_weight_rationale_report.ipynb` | 실제 M1 output 기준 0.22/0.78/0.92. 0.44는 active threshold로 말하지 않음 |
| fault_group_weight 제한 | `docs/model/MODEL_INVENTORY_KO.md`, `docs/handoff/AGENT_HANDOFF_KO.md` | live inference에서 label-derived risk 검토 필요 |
| gate threshold 제한 | `docs/model/MODEL_INVENTORY_KO.md`, `docs/handoff/AGENT_HANDOFF_KO.md` | 0.50/0.60은 evidence runtime policy. task/activity native label 부재 |
...
- 추가 필요 항목:
  ...
  - 추가 위치: `docs/model/MODEL_INVENTORY_KO.md`, `compare/m1_threshold_weight_rationale_report.ipynb`, M1 specialist runtime policy 문서.
...
- 추가 필요 항목:
  ...
  - 추가 위치: `compare/m1_threshold_weight_rationale_report.ipynb`, `docs/model/MODEL_INVENTORY_KO.md`.
