...
이 문서는 운영 에이전트 반복 판단, 모델 재검증, 읽기 전용 진단, 사람 검수, 재학습, 모델 승격의 전체 흐름을 설명한다. 외부 웹 검색 실행 capability는 폐기되었다.
...
flowchart TD
    ...
    L --> K
    K -->|"긴급·높음·검수·모델 불일치"| M["읽기 전용 진단 worker"]
    M --> O
...
4. 부족한 원인에 따라 내부 검색 확대, 모델 재실행, 읽기 전용 진단 중 하나를 선택한다.
...
- 최대 반복 횟수
- 외부 웹 검색·URL·도메인·검색어 capability 금지
- 모델 불일치 시 재검증 우선
- 고위험 항목의 사람 검수
- 최종 결과의 사람 검수
...
에이전트 Tool은 LLM이 선택할 수 있는 읽기 전용 Tool 8개와 보고서 통제 Tool로 분리한다.
...
LLM과 진단 worker는 외부 검색, PostgreSQL 쓰기, 보고서 파일 쓰기를 직접 호출할 수 없다. 진단 worker는 frozen ML·날씨·내부 RAG snapshot과 모델 포트만 받는다.
...
| `evidence_candidates` | 운영자 수동 근거 후보와 승인·RAG 적재 상태; 과거 외부 검색 row는 조회 전용 |
...
새로 생성할 수 있는 근거는 ML 모델 데이터, Substation과 시간 구간으로 고정한 구조화 날씨, 내부 RAG, 운영자 수동 근거다. 외부 웹 검색 evidence, URL·도메인·검색어 기반 조회, `external_search` 승인·실행 task는 새로 만들 수 없다.

운영자 수동 근거는 기존 검수와 RAG 적재 경로를 유지한다. 기존 `source_type=web`, query 포함 candidate, `external_search` task는 historical response 역직렬화를 위해 조회할 수 있지만 수정·승인·재실행·RAG 적재할 수 없다.
