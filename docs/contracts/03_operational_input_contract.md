...
- `priority_scores`: priority LGBM 회귀모델의 운영 큐 점수/등급

`model_chain_output`은 `priority_scores`의 직접 입력이므로 DDL 실행 순서도 `006_model_chain_output.sql` 다음 `007_priority_scores.sql`로 둔다. 모델 갱신과 스키마 변경을 분리하기 위해 priority에는 `model_version`을 둔다.
