...
> 현재 runtime 기준 주의: 이 문서는 priority LGBM 회귀모델을 채택하던 시점의 legacy 성능 검토 기록이다. 2026-06-26 현재 proto runtime은 IF + LGBM risk + LGBM leadtime까지는 유지하고, priority 단계만 `priority_engine_v2_rule_based_tuned` 규칙 기반 엔진으로 실행한다.
...
- priority LGBM을 다시 검토하려면 운영 라벨이 확보된 뒤 별도 실험으로 재학습해야 한다. 현재 runtime priority는 규칙 기반 엔진이다.
