...
| 구분 | 현재 상태 | 판단 |
|---|---|---|
| 프로토 목표 | `raw -> preprocessing -> IF + LGBM risk + LGBM leadtime -> priority LGBM regression -> server -> frontend` 체인 연결 | 완료 |
...
| ML 모델 | priority 회귀모델 | `agent/priority/train_priority_model.py`, `agent/priority/models/*` | full 기준 3346 rows | priority LGBMRegressor 학습/저장 |
| ML 모델 | priority 추론/검증 | `agent/priority/run_priority.py`, `evaluate.py`, `validate_contracts.py` | current 300 x 9 | priority score/level 생성과 schema 검증 |
...
| pytest | 14 passed | `model_chain_output` JSON Schema E2E 검증 추가 후 재실행 |
...
| priority 입력 feature | 7 | anomaly/risk/leadtime 확률 기반 |
| priority 학습 기준 row | 3346 | full PreDist supervised chain output 기준 |
| priority train / holdout | 2509 / 837 | `substation_id % 3 == 0` deterministic holdout |
| 기존 300행 모델 binary F1 | 0.4615 | fixture만으로 학습한 기존 모델은 부족 |
| full 학습 모델 mock raw binary F1 | 0.8511 | mock raw holdout에서 크게 개선 |
| full 학습 모델 full holdout weighted F1 | 0.4857 | 운영 보증값은 아니지만 프로토 채택 근거 |
| priority 모델 판정 | 채택 가능 | 운영 큐 정렬 목적 기준, 운영 보증 모델은 아님 |
...
| ML 모델 | 체인 연결과 priority 채택 가능성 증명 | 운영 split, drift monitoring, 재학습 트리거 |
...
| 테스트 재실행 | `uv run pytest` 통과, 14 passed |
...
- `priority_model_metadata.json`의 `training_basis`는 `data/processed/ml_model_chain/model_chain_output.csv`로 기록되어 있지만, 현재 같은 경로의 CSV는 mock raw 실행으로 덮여 있다. 재현성을 높이려면 alpha 전환 전에 학습용 full chain output과 serving용 mock/current output 경로를 분리하는 편이 안전하다.
...
| 순서 | 다음 작업 | 이유 |
|---:|---|---|
| 1 | 학습용 full chain output과 serving output 경로 분리 | 모델 재현성 혼동 방지 |
