...
| 구분 | 경로 | 설명 |
|---|---|---|
| 입력 | `preprocessed_windows_sample.csv` | 300행 x 211컬럼 |
| feature report | `data/processed/ml_model_chain/feature_adapter_report.json` | 모델별 feature 매핑과 0-fill 기록 |
| 출력 | `data/processed/ml_model_chain/model_chain_output.csv` | risk, leadtime, anomaly 신호 300행 |
...
| risk medium | 33 |
| risk low | 111 |
| predicted leadtime 0-24h | 105 |
| predicted leadtime 1-3d | 180 |
| predicted leadtime 3-7d | 15 |
...
```mermaid
flowchart TD
  PRE["preprocessed_windows<br/>300 x 211"] --> ADAPT["feature adapter"]
  ...
  AO --> OUT["model_chain_output.csv<br/>300 x 25"]
  RO --> OUT
  LO --> OUT
```
