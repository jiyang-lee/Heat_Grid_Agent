...
원칙: **기존 백엔드는 고정(수정 금지), 계약(`/api` 경로)도 무변경.** 프론트에서만 대응.
...
## 2. 토큰/비용이 항상 $0 → 백엔드 고정 원칙에 따라 $0 수용
...
### 결정
- **백엔드 고정(수정 금지) 원칙을 유지**하므로 UI의 토큰/비용은 `$0`으로 둔다(현 구조상 정상 동작).
- 실제로 표시하려면 `nodes.py`가 LLM `usage_metadata`를 `TokenUsage.calls`에 기록하도록 하는 백엔드 최소 수정이 필요하다(계약·스키마 무변경, 값만 채움) — 고정 원칙 해제 시 별도 처리.
...
| 토큰/비용 $0 | ❌ 백엔드 집계 누락이라 불가 | 백엔드 고정, $0 수용 |
