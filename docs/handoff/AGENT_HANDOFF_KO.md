...
| `output/reports/missing_agent_windows.csv` | final card 누락 점검 결과(현재 0행) |
...
| 파일 | 역할 | rows | columns |
|---|---|---:|---:|
| `output/agent_priority_card.csv` | agent/API/UI가 우선 읽는 official card | 1252 | 67 |
| `output/agent/m1_agent_priority_card.csv` | official card 복사본 | 1252 | 67 |
...
| 컬럼 | 의미 |
|---|---|
| `priority_score` | 최종 M1 restored Risk/pre-event gate v4 priority score |
...
| 파일 | 설명 |
|---|---|
| `output/agent/agent_card_column_groups_ko.md` | 67개 final card 컬럼과 29개 parallel card 컬럼 분류 |
| `output/reports/row_flow_summary.csv` | canonical 1252 -> final 1252 row 흐름 |
