    ...
    async with engine.connect() as connection:
        run_result = await connection.execute(
            text(
                ...
                "count(*) FILTER (WHERE worker_status = 'completed') "
                "AS diagnostic_completed_count, "
                "count(*) FILTER (WHERE worker_status = 'timeout') "
                "AS diagnostic_timeout_count, "
                "count(*) FILTER (WHERE worker_status = 'invalid') "
                "AS diagnostic_invalid_count, "
                "count(*) FILTER (WHERE worker_status = 'budget_exceeded') "
